...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description="Direct Model Training - EfficientNet 전용")
    parser.add_argument("--model", default="efficientnet", choices=["efficientnet"], 
                       help="훈련할 모델 (기본값: efficientnet)")
    ...
    # EfficientNet 훈련
    print("🚀 EfficientNet 모델 훈련 중...")
    results_eff, save_dir_eff = train_efficientnet()
    
    print(f"\n✅ EfficientNet 훈련 완료!")
    print(f"📂 저장 위치: {save_dir_eff}")
    print(f"📊 최종 성능: Train Loss: {results_eff['train_losses'][-1]:.4f}, "
          f"Val Loss: {results_eff['val_losses'][-1]:.4f}")
    
    print("\n🎯 다음 단계: python scripts/3_convert_to_coreml.py 를 실행하여 Core ML로 변환하세요!")
